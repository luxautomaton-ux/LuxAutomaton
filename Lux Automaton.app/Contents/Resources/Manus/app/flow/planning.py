...
class PlanningFlow(BaseFlow):
    ...
    async def execute(
        ...
        try:
            ...
            result = ""
            while True:
                # Check whether execution was cancelled by user
    ...
    async def _create_initial_plan(self, request: str, job_id: str = None) -> None:
        ...
        # Use provided job_id or derive one from request
        if not job_id:
            job_id = f"job_{request[:8].replace(' ', '_')}"
            if len(job_id) < 10:  # If too short, fall back to timestamp
        ...
        # Bind log output to the job-specific log file
        logger.add(log_file_path, rotation="100 MB")

        logger.info(f"Creating initial plan with ID: {self.active_plan_id}")

        # Continue regular plan creation flow
        ...
        if response.tool_calls:
            ...
            # If we are here, it means we had tool calls but not the 'planning' tool
            logger.warning("LLM called tools but not the planning tool")
        else:
            logger.warning("LLM did not provide any tool calls for planning")
