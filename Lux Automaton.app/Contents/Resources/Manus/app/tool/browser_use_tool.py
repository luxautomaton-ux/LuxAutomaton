...
class BrowserUseTool(BaseTool):
    ...
    def __del__(self):
        """Attempt resource cleanup during object destruction."""
        if hasattr(self, "browser") and self.browser is not None:
            try:
                ...
                try:
                    ...
                    if loop.is_running():
                        # Skip cleanup to avoid runtime errors inside active loop
                        logging.warning("Event loop is running; skipping browser cleanup to avoid runtime errors.")
                        return
                except RuntimeError:
                    # No running loop; create a temporary loop for cleanup
