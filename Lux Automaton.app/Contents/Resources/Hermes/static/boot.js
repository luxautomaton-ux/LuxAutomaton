...
function syncWorkspacePanelState(){
  ...
  // Even without a session, we allow the panel to stay in its current mode
  // (likely 'browse') so users can see the file tree or workspace placeholder.
  if(_workspacePanelMode==='preview' && !S.session) {
...
function syncWorkspacePanelUI(){
  ...
  const canBrowse=true; // Always allow opening the workspace panel to browse files
  const hasPreview=_hasWorkspacePreviewVisible();
  if(toggleBtn){
