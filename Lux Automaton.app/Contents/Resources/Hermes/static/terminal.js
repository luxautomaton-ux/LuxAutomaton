...
const TERMINAL_STATE_KEY='hermes-terminal-state';

function _saveTerminalState(){
  try{
    localStorage.setItem(TERMINAL_STATE_KEY, JSON.stringify({
      open:TERMINAL_UI.open,
      collapsed:TERMINAL_UI.collapsed,
      height:TERMINAL_UI.height,
    }));
  }catch(_){}
}
...
function _terminalEls(){
  return {
    ...
    toggleMobile:$('btnTerminalToggleMobile'),
...
function syncTerminalButton(){
  const {toggle,toggleMobile}= _terminalEls();
  ...
  [toggle,toggleMobile].forEach(btn=>{
    ...
    btn.classList.toggle('active',TERMINAL_UI.open);
    btn.setAttribute('aria-pressed',TERMINAL_UI.open?'true':'false');
    btn.title=hasWorkspace?(TERMINAL_UI.collapsed?t('terminal_expand'):t('terminal_open_title')):t('terminal_no_workspace_title');
...
(async function _restoreTerminalState(){
  try{
    const raw=localStorage.getItem(TERMINAL_STATE_KEY);
    if(!raw)return;
    const state=JSON.parse(raw);
    if(!state)return;
    if(state.height) TERMINAL_UI.height=state.height;
    if(state.open){
      // Brief delay to ensure session and DOM are ready
      setTimeout(async ()=>{
        if(S.session && S.session.workspace){
          await toggleComposerTerminal(true);
          if(state.collapsed) collapseComposerTerminal();
        }
      }, 800);
    }
  }catch(_){}
