...
async function switchPanel(name, opts = {}) {
  ...
  if (nextPanel === 'swarm') await loadSwarmStatus();
...
async function _kanbanPopulateAssigneeSelect(currentValue){
  ...
  let html = '';
  if (profiles.length) {
    html += `<optgroup label="${esc(t('kanban_assignee_profiles_label') || 'Lux Automaton profiles')}">`;
...
async function loadSettingsPanel(){
  try{
    ...
    const botNameField=$('settingsBotName');
    if(botNameField){
      botNameField.value=settings.bot_name||'Lux Automaton';
...
function _applySavedSettingsUi(saved, body, opts){
  ...
  window._botName=body.bot_name||'Lux Automaton';
  if(typeof applyBotName==='function') applyBotName();
...
async function saveSettings(andClose){
  ...
  body.bot_name=botName||'Lux Automaton';
...
// ── Swarm panel ──
let _swarmPollingInterval = null;

async function loadSwarmStatus(forceRefresh = false) {
  const listEl = $('swarmList');
  const statsEl = $('swarmStats');
  if (!listEl || !statsEl) return;
  try {
    const data = await api('/api/swarm/status');
    if (!data || !data.ok) {
      listEl.innerHTML = '<div style="color:var(--error);font-size:12px">Failed to load swarm status</div>';
      return;
    }
    const counts = data.counts || { running: 0, completed: 0, cancelled: 0 };
    statsEl.innerHTML = `<span>Running: ${counts.running}</span> | <span>Completed: ${counts.completed}</span> | <span>Cancelled: ${counts.cancelled}</span>`;
    const missions = data.missions || [];
    if (missions.length === 0) {
      listEl.innerHTML = '<div style="color:var(--muted);font-size:12px;padding:12px">No missions yet. Click "New Mission" to start.</div>';
      return;
    }
    let html = '';
    for (const m of missions) {
      const statusClass = m.status === 'running' ? 'swarm-status-running' : (m.status === 'completed' ? 'swarm-status-completed' : 'swarm-status-cancelled');
      const created = new Date((m.created_at || 0) * 1000).toLocaleString();
      html += `<div class="swarm-mission-item" style="padding:12px;border-bottom:1px solid var(--border);cursor:pointer" onclick="showSwarmMissionDetail('${esc(m.mission_id)}')">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:4px">
          <span style="font-weight:600;font-size:13px">${esc(m.prompt || '').substring(0, 40)}${m.prompt && m.prompt.length > 40 ? '...' : ''}</span>
          <span class="${statusClass}" style="font-size:11px;padding:2px 6px;border-radius:4px;background:var(--bg-secondary)">${esc(m.status)}</span>
        </div>
        <div style="font-size:11px;color:var(--muted)">Workers: ${m.worker_count} | Created: ${created}</div>
      </div>`;
    }
    listEl.innerHTML = html;
  } catch (e) {
    listEl.innerHTML = '<div style="color:var(--error);font-size:12px">' + esc(e.message) + '</div>';
  }
}

async function showSwarmMissionDetail(missionId) {
  try {
    const data = await api('/api/swarm/mission?mission_id=' + encodeURIComponent(missionId));
    ...
    const modal = showModal({
      title: 'Mission: ' + (m.prompt || '').substring(0, 50),
      body: `<div style="padding:12px;font-size:13px">
        <div style="margin-bottom:12px"><strong>Status:</strong> ${esc(m.status)}</div>
        <div style="margin-bottom:12px"><strong>Workers:</strong> ${m.worker_count}</div>
        <div style="margin-bottom:12px"><strong>Workspace:</strong> ${esc(m.workspace || 'None')}</div>
        <div style="margin-bottom:12px"><strong>Prompt:</strong><pre style="background:var(--bg-secondary);padding:8px;border-radius:4px;margin-top:4px;white-space:pre-wrap">${esc(m.prompt || '')}</pre></div>
        ${m.git && m.git.is_repo ? `<div style="margin-bottom:12px"><strong>Git:</strong> ${esc(m.git.branch)} (${esc(m.git.head)})</div>` : ''}
        <div style="margin-bottom:12px"><strong>Events:</strong></div>
        <div style="max-height:200px;overflow-y:auto">${(m.events || []).map(e => `<div style="padding:4px 0;border-bottom:1px solid var(--border);font-size:11px"><span style="color:var(--muted)">${new Date(e.ts * 1000).toLocaleTimeString()}</span> <strong>${esc(e.kind)}</strong>: ${esc(e.message)}</div>`).join('')}</div>
      </div>`,
      actions: [
        { label: 'Close', onclick: () => closeModal() },
        ...(m.status === 'running' ? [{ label: 'Cancel', danger: true, onclick: () => cancelSwarmMission(missionId) }] : []),
        { label: 'Delete', danger: true, onclick: () => deleteSwarmMission(missionId) }
      ]
    });
  } catch (e) {
...
async function cancelSwarmMission(missionId) {
  try {
    const data = await api('/api/swarm/cancel', { method: 'POST', body: JSON.stringify({ mission_id: missionId }) });
    if (data && data.ok) {
      showToast('Mission cancelled');
      loadSwarmStatus();
    } else {
      showToast((data && data.error) || 'Failed to cancel', 'error');
    }
  } catch (e) {
...
async function deleteSwarmMission(missionId) {
  const ok = await showConfirmDialog({ title: 'Delete Mission', message: 'Are you sure you want to delete this mission?', confirmLabel: 'Delete', danger: true });
  if (!ok) return;
  try {
    const data = await api('/api/swarm/delete', { method: 'POST', body: JSON.stringify({ mission_id: missionId }) });
    if (data && data.ok) {
...
function openSwarmNewMission() {
  const modal = showModal({
    title: 'New Swarm Mission',
    body: `<div style="padding:12px">
      <div style="margin-bottom:12px">
        <label style="display:block;font-size:12px;margin-bottom:4px">Mission Prompt</label>
        <textarea id="swarmPromptInput" rows="4" style="width:100%;background:var(--input-bg);color:var(--text);border:1px solid var(--border);border-radius:6px;padding:8px;font-size:13px" placeholder="Describe what you want the swarm to accomplish..."></textarea>
      </div>
      <div style="margin-bottom:12px">
        <label style="display:block;font-size:12px;margin-bottom:4px">Workers (1-12)</label>
        <input type="number" id="swarmWorkersInput" value="3" min="1" max="12" style="width:100%;background:var(--input-bg);color:var(--text);border:1px solid var(--border);border-radius:6px;padding:8px;font-size:13px">
      </div>
      <div style="margin-bottom:12px">
        <label style="display:block;font-size:12px;margin-bottom:4px">Workspace (optional)</label>
        <input type="text" id="swarmWorkspaceInput" style="width:100%;background:var(--input-bg);color:var(--text);border:1px solid var(--border);border-radius:6px;padding:8px;font-size:13px" placeholder="Leave empty for default">
      </div>
      <div>
        <label style="font-size:12px"><input type="checkbox" id="swarmAutoMerge"> Auto-merge results</label>
      </div>
    </div>`,
    actions: [
      { label: 'Cancel', onclick: () => closeModal() },
      { label: 'Start Mission', primary: true, onclick: () => startSwarmMission() }
    ]
  });
}

async function startSwarmMission() {
  const prompt = $('swarmPromptInput')?.value?.trim();
  if (!prompt) {
    showToast('Please enter a mission prompt', 'error');
    return;
  }
  const workers = parseInt($('swarmWorkersInput')?.value || '3', 10);
  const workspace = $('swarmWorkspaceInput')?.value?.trim() || '';
  const auto_merge = $('swarmAutoMerge')?.checked || false;
  try {
    const data = await api('/api/swarm/start', {
      method: 'POST',
      body: JSON.stringify({ prompt, workers, workspace: workspace || null, auto_merge })
    });
    if (data && data.ok) {
      showToast('Mission started: ' + (data.mission?.mission_id || '').substring(0, 8));
      closeModal();
      loadSwarmStatus();
    } else {
      showToast((data && data.error) || 'Failed to start mission', 'error');
    }
  } catch (e) {
    showToast(e.message, 'error');
  }
