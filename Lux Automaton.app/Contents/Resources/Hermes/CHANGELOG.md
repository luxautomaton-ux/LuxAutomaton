...
- **Native Hermes Swarm** — New `/api/swarm/*` endpoints and Swarm panel in the WebUI for mission lifecycle management (start, status, cancel, complete, delete missions) with native git workspace snapshots, worker planning (8-role rotation), and feature scanning. Backend implementation in `api/swarm.py`, frontend in panels.js with full i18n support.
