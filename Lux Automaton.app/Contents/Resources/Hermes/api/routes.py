...
def handle_get(handler, parsed) -> bool:
    ...
    if parsed.path.startswith("/api/swarm"):
        from api.swarm import handle_swarm_get

        result = handle_swarm_get(handler, parsed)
        if result is False:
            return bad(handler, f"unknown swarm endpoint: GET {parsed.path}", status=404)
        return True
...
def handle_post(handler, parsed) -> bool:
    ...
    # ── Swarm mission state ──
    if parsed.path.startswith("/api/swarm"):
        from api.swarm import handle_swarm_post

        result = handle_swarm_post(handler, parsed, body)
        if result is False:
            return bad(handler, f"unknown swarm endpoint: POST {parsed.path}", status=404)
        return True
