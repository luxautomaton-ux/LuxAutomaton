  ...
  if (priority === 'low') {
    return (
      <Tooltip content="Low priority">
        <div className="flex h-4 w-4 items-center justify-center rounded bg-orange-100 dark:bg-orange-900/30">
          <Circle className="h-1.5 w-1.5 fill-current text-orange-600 dark:text-orange-400" />
        </div>
      </Tooltip>
    );
  }
  ...
  return (
    <div
      className={cn(
        ...
        'hover:shadow-md hover:border-orange-300 dark:hover:border-orange-600 transition-all duration-200',
        ...
        <div className="ml-3">
          <div className="mb-1 flex items-center gap-2">
            ...
            <div className="h-1.5 flex-1 rounded-full bg-gray-200 dark:bg-gray-700" title={`${progress.completed} of ${progress.total} subtasks completed`}>
              <div
                className={cn('h-full rounded-full transition-all duration-300', task.status === 'done' ? 'bg-green-500' : 'bg-orange-500')}
