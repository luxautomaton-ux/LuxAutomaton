...
export default function FileTree({ selectedProject, onFileOpen }: FileTreeProps) {
  ...
  return (
    ...
    >
      {/* Drag overlay */}
      {upload.isDragOver && (
        <div className="absolute inset-0 z-50 flex items-center justify-center border-2 border-dashed border-orange-500 bg-orange-500/10">
        ...
        {operations.isCreating && (
          ...
          >
            {operations.newItemType === 'directory' ? (
              <Folder className={cn(ICON_SIZE_CLASS, 'text-orange-500')} />
            ) : (
