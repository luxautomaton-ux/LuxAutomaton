    ...
    <div ref={containerRef} className={`flex h-full min-w-0 flex-shrink-0 ${editorExpanded ? 'flex-1' : ''}`}>
      {!editorExpanded && (
        <div
          ...
          className="group relative w-1 flex-shrink-0 cursor-col-resize bg-gray-200 transition-colors hover:bg-orange-500 dark:bg-gray-700 dark:hover:bg-orange-600"
          title="Drag to resize"
        >
          <div className="absolute inset-y-0 left-1/2 w-1 -translate-x-1/2 bg-orange-500 opacity-0 transition-opacity group-hover:opacity-100 dark:bg-orange-600" />
        </div>
      )}
