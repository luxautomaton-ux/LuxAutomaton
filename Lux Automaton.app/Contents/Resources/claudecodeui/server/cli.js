 ...
 * Lux CLI CLI
 *
 * Provides command-line utilities for managing Lux CLI
 *
 * Commands:
...
// Match the runtime fallback in load-env.js so "luxcli status" reports the same default
// database location that the backend will actually use when no DATABASE_PATH is configured.
const DEFAULT_DATABASE_PATH = path.join(os.homedir(), '.luxcli', 'auth.db');
...
function showStatus() {
    ...
    console.log('\n' + c.dim('═'.repeat(60)));
    console.log(`\n${c.tip('[TIP]')} Hints:`);
console.log(`      ${c.dim('>')} Use ${c.bright('luxcli --port 8080')} to run on a custom port`);
      console.log(`      ${c.dim('>')} Use ${c.bright('luxcli --database-path /path/to/db')} for custom database`);
      console.log(`      ${c.dim('>')} Run ${c.bright('luxcli help')} for all options`);
...
function showHelp() {
    console.log(`
╔═══════════════════════════════════════════════════════════════╗
║              Lux CLI - Command Line Tool               ║
╚═══════════════════════════════════════════════════════════════╝

Usage:
  claude-code-ui [command] [options]
luxcli [command] [options]

  start          Start the Lux CLI server (default)
  sandbox        Manage Docker sandbox environments
  status         Show configuration and data locations
...
Examples:
  $ luxcli                        # Start with defaults
  $ luxcli --port 8080            # Start on port 8080
  $ luxcli sandbox ~/my-project   # Run in a Docker sandbox
  $ luxcli status                 # Show configuration
...
Documentation:
  ${packageJson.homepage || 'https://github.com/lux-automaton/LuxCLI'}

Report Issues:
  ${packageJson.bugs?.url || 'https://github.com/lux-automaton/LuxCLI/issues'}
`);
}
...
async function checkForUpdates(silent = false) {
    try {
        ...
        const latestVersion = execSync('npm show @lux-automaton/lux-cli version', { encoding: 'utf8' }).trim();
        const currentVersion = packageJson.version;

        if (isNewerVersion(latestVersion, currentVersion)) {
            console.log(`\n${c.warn('[UPDATE]')} New version available: ${c.bright(latestVersion)} (current: ${currentVersion})`);
            console.log(`         Run ${c.bright('luxcli update')} to update\n`);
...
execSync('npm update -g @lux-automaton/lux-cli', { stdio: 'inherit' });
    console.log(`${c.ok('[OK]')} Update complete! Restart luxcli to use the new version.`);
    } catch (e) {
        console.error(`${c.error('[ERROR]')} Update failed: ${e.message}`);
        console.log(`${c.tip('[TIP]')} Try running manually: npm update -g @lux-automaton/lux-cli`);
    }
}
...
function showSandboxHelp() {
    console.log(`
${c.bright('Lux CLI Sandbox')} — Run Lux CLI inside Docker Sandboxes
${c.bright('luxcli sandbox <workspace>')}            Create and start a sandbox
${c.bright('luxcli sandbox <subcommand> [name]')}    Manage sandboxes
...
Subcommands:
  ...
  ${c.bright('logs')}         Show Lux CLI server logs
...
Examples:
$ luxcli sandbox ~/my-project
$ luxcli sandbox ~/my-project --agent codex --port 8080
$ luxcli sandbox ~/my-project --env SERVER_PORT=8080 --env HOST=0.0.0.0
$ luxcli sandbox ls
$ luxcli sandbox stop my-project
$ luxcli sandbox start my-project
$ luxcli sandbox rm my-project
...
Advanced usage:
  ...
  or passing prompts to the agent, use sbx directly with the template:
    ...
    sbx run --template local claude ~/my-project --branch my-feature
    sbx run --template local claude ~/project ~/libs:ro --memory 8g
...
async function sandboxCommand(args) {
    ...
    switch (opts.subcommand) {
        ...
        case 'stop':
            if (!opts.name) {
                console.error(`\n${c.error('❌')} Sandbox name required: luxcli sandbox stop <name>\n`);
                process.exit(1);
            }
        ...
        case 'rm':
            if (!opts.name) {
                console.error(`\n${c.error('❌')} Sandbox name required: luxcli sandbox rm <name>\n`);
                process.exit(1);
            }
        ...
        case 'logs':
            if (!opts.name) {
                console.error(`\n${c.error('❌')} Sandbox name required: luxcli sandbox logs <name>\n`);
                process.exit(1);
            }
            try {
                sbx(['exec', opts.name, 'bash', '-c', 'cat /tmp/luxcli.log'], { inherit: true });
            } catch (e) {
        ...
        case 'start': {
            if (!opts.name) {
                console.error(`\n${c.error('❌')} Sandbox name required: luxcli sandbox start <name>\n`);
                process.exit(1);
            }
            ...
            console.log(`${c.info('▶')} Launching Lux CLI web server...`);
            sbx(['exec', opts.name, 'bash', '-c', 'luxcli start --port 3001 &']);
            ...
            console.log(`\n${c.ok('✔')} ${c.bright('Lux CLI is ready!')}`);
        ...
        case 'create': {
            if (!opts.workspace) {
                console.error(`\n${c.error('❌')} Workspace path required: luxcli sandbox <path>\n`);
                console.log(`   Example: ${c.bright('luxcli sandbox ~/my-project')}\n`);
                process.exit(1);
            }
            ...
            // Step 3: Start Lux CLI inside the sandbox
            console.log(`${c.info('▶')} Launching Lux CLI web server...`);
            sbx(['exec', opts.name, 'bash', '-c', 'luxcli start --port 3001 &']);
            ...
            console.log(`\n${c.dim('  Or install globally:')} npm install -g @lux-automaton/lux-cli\n`);
            break;
        }
...
async function main() {
    ...
    switch (command) {
        ...
        default:
            ...
            console.log('   Run "luxcli help" for usage information.\n');
            process.exit(1);
    }
}
