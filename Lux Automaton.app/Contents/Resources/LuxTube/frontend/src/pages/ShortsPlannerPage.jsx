...
export default function ShortsPlannerPage() {
  ...
  async function generatePlan() {
    ...
    try {
      const data = await api('/api/strategy/generate', {
        method: 'POST',
        body: JSON.stringify({ mode: 'shorts', channels: channelList }),
      })
      setPlan(data)
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="font-headline-md text-headline-md text-on-surface mb-1">Shorts Planner</h1>
          <p className="font-body-md text-body-md text-on-surface-variant">Generate repeatable short-form hooks and execution checklist.</p>
        </div>
        <button onClick={generatePlan} disabled={loading} className="bg-inverse-primary text-white font-label-caps text-label-caps px-6 py-3 rounded-xl font-bold hover:bg-primary-container transition-colors flex items-center gap-2 shadow-glow-red disabled:opacity-50">
          <span className={`material-symbols-outlined text-[16px] ${loading ? 'animate-spin' : ''}`}>{loading ? 'progress_activity' : 'auto_awesome'}</span>
          {loading ? 'GENERATING...' : 'GENERATE SHORTS PLAN'}
        </button>
      </div>
          ...
          <div className="bg-surface-container rounded-2xl border border-outline-variant/30 shadow-card overflow-hidden">
            <div className="px-5 py-3 border-b border-outline-variant/20 bg-surface-container-high/30">
              <h3 className="font-label-caps text-label-caps text-on-surface flex items-center gap-2">
                <span className="material-symbols-outlined text-[16px] text-primary">list</span> SOURCE CHANNELS
              </h3>
            </div>
            <div className="p-5">
              <textarea
                ...
                className="w-full h-40 bg-surface-container-lowest border border-outline-variant/20 rounded-xl p-3 text-[13px] text-on-surface font-data-mono focus:border-primary outline-none resize-none"
        ...
        <div className="lg:col-span-8">
          {!plan && !loading ? (
            <div className="bg-surface-container rounded-2xl border border-outline-variant/30 shadow-card p-12 text-center">
              <span className="material-symbols-outlined text-[48px] text-on-surface-variant opacity-20">vertical_split</span>
              <p className="font-data-mono text-data-mono text-on-surface-variant mt-3">Generate a shorts strategy from your channel evidence</p>
            </div>
          ) : plan && (
            <div className="bg-surface-container rounded-2xl border border-outline-variant/30 shadow-card overflow-hidden">
